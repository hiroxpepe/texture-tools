...
#nullable enable
        ...
        public static void Execute() {
            Cut cut1 = NewCutByPiece(piece_count_x: 8, piece_count_y: 8);
            Face face1 = NewFace(width: 512, hight: 512, cut: cut1);
            Cut cut2 = NewCutByPiece(piece_count_x: 5, piece_count_y: 5);
            Face face2 = NewFace(width: 512, hight: 512, cut: cut2);
            ...
            face1.OnReady += () => {
                ...
                face1.AllPoint.ForEach(action: x => {
                    x.Where(predicate: x => !x.FixedX).ToList().ForEach(action: x => {
                        if (!x.MovedX) { x.X += _random.Next(minValue: -12, maxValue: 12); x.MovedX = true; }
                    });
                    x.Where(predicate: x => !x.FixedY).ToList().ForEach(action: x => {
                        if (!x.MovedY) { x.Y += _random.Next(minValue: -12, maxValue: 12); x.MovedY = true; }
                    });
                });
                // fills by color.
                ...
                face1.AllPoint.ForEach(action: x => {
                    ...
                    if (random_number == 1) {
                        tool.Fill(points: x, color: Color.Magenta, img_idx: img_idx, cell_idx: count, debug: false);
                    }
                    count++;
                });
            };
